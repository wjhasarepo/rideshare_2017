...
function f3(addr) {
  console.log("function f3 with parameter " + addr);
...
/*
let retrieveFromRequest = new Promise(function(resolve, reject) {
...
function f2(param) {
  return new Promise(function(resolve, reject) {
    connection.query("SELECT * FROM rides_offered where start_address = '"+param.passengerStartAddress +"'", function(err, rows){
      if(err) {
        var error = new Error("Error Selecting : %s", err);
        reject(error)
      } else {
        ...
        for(row of rows) {
          ...
          resolve([driverStartAddress, driverDestinationAddress, offerTime]);
        }
      }
    }
  }
